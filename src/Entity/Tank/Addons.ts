...
export class Addon {
    ...
    protected createHomingAutoTurrets(count: number) {
        const rotPerTick = AI.PASSIVE_ROTATION;
        const MAX_ANGLE_RANGE = PI2 / 4; // keep within 90º each side
        ...
        for (let i = 0; i < count; ++i) {
            const base = new AutoTurret(rotator, HomingAutoTurretMiniDefinition);
            base.influencedByOwnerInputs = true;
...
const HomingAutoTurretMiniDefinition: BarrelDefinition = {
    angle: 0,
    offset: 0,
    size: 55,
    width: 42 * 0.7,
...
class HomingAuto extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);
